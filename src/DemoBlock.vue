<template>
  ...
  >
    ...
    <div
      ...
      :style="{ 'width': fixedControl ? `${codeContentWidth + 1}px` : '100%' }"
      ref="control"
...
export default {
  ...
  computed: {
    ...
    demoAreaHeight() {
      return this.$el.getElementsByClassName("demo-content")[0].clientHeight;
    }
  },
  methods: {
    copyCode() {
  ...
  watch: {
    isExpanded(val) {
      this.codeArea.style.height = val ? `${Math.max((this.codeAreaHeight + 1), this.demoAreaHeight)}px` : `${this.demoAreaHeight}px`;
      if (!val) {
  ...
  mounted() {
    this.$nextTick(() => {
      let metaContent = this.$el.getElementsByClassName("meta")[0];
      metaContent.style.height = `${this.demoAreaHeight}px`;
      let codeContent = this.$el.getElementsByClassName('code-content')[0];
      this.codeContentWidth = codeContent.clientWidth
      if (this.$el.getElementsByClassName('description').length === 0) {
